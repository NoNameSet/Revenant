#ifndef REVENANT_ATFIXEDARRAY_H
#define REVENANT_ATFIXEDARRAY_H
...
#pragma pack(push, 8)
    ...
    public:
...
#if _WIN32
        atFixedArray(const atFixedArray& right)
        {
        ...
        atFixedArray(void* data_ptr, std::uint16_t size, std::uint16_t count) :
                m_data(data_ptr),
                m_size(size),
                m_count(count)
        {
        ...
        bool append(atFixedArray<T, maxCount> value_array)
        {
        ...
        bool remove(T value)
        {
            for (auto iter_value : *this)
            {
                if (iter_value == value)
                {
                    auto removed = m_data[value];
                    for (int i = value; i < (m_count - 1); i++)
                    {
                        m_data[i] = m_data[i + 1];
                    }
                    m_count--;
        ...
        friend std::ostream& operator<<(std::ostream& o, const atFixedArray<T, maxCount>& j)
        {
